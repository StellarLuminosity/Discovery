...
class DiscoveryGuy:
    ...
    def __init__(self, **kwargs):
        ...
        self.project_source = self.kwargs.get('project_source') or self.kwargs.get('target_source_dir')
        self.backup_seeds_vault = self.kwargs['backup_seeds_vault']
        self.report_dir = self.kwargs['report_dir']
        self.fuzzers_sync_base = self.kwargs.get('fuzzers_sync_base', '')
        ...
        self.target_source_dir = self.kwargs.get('target_source_dir') or self.project_source
        self.target_build_command = self.kwargs.get('target_build_command', '')
        self.target_run_command = self.kwargs['target_run_command']
        self.target_harness_name = self.kwargs.get('target_harness_name', 'local_harness')
        self.target_harness_function_name = self.kwargs.get('target_harness_function_name', 'LLVMFuzzerTestOneInput')
        self.target_harness_source_path = self.kwargs.get('target_harness_source_path', '')
        self.target_crash_keywords = self.kwargs.get('target_crash_keywords', [])
        if not self.project_source or not self.target_source_dir:
            raise ValueError("Missing source path. Set target.source_dir in standalone_config.py.")
        self.fuzz_request = {}
        ...
        ########################################################
        # 📦 INSTANTIATE LOCAL TARGET PROJECT
        ########################################################
        local_target = TargetProject(
            source_dir=self.target_source_dir,
            build_command=self.target_build_command,
            run_command=self.target_run_command,
            harness_function_name=self.target_harness_function_name,
            harness_source_path=self.target_harness_source_path,
            crash_keywords=self.target_crash_keywords,
        )
        local_target.build_target()
        self.cps_debug = [local_target]

        ########################################################
        # 🏖️ INSTANTIATE SANDBOX AND CRASH CHECKER
        ########################################################
        # Local sandbox/runtime for generated scripts.
        self.sandbox = self.cps_debug[0]
    ...
    def exit_and_clean(self, code):
        try:
            for cp in getattr(self, "cps_debug", []):
                cleanup_fn = getattr(cp, "cleanup", None)
